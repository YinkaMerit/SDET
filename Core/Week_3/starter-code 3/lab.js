...
if (drunk_guess == "true" || parseInt(age_guess)< 30) {
  alert("She smiles and shrugs. 'I'll never tell!' Phew, that was a close one!");
}else if (drunk_guess != "true" || parseInt(age_guess)>= 30) {
...
if (pay_choice == "2" && parseInt(money_in_wallet)>= 100) {
  alert("How very gallant of you! She seems impressed...");
}else (pay_choice != "2" || parseInt(money_in_wallet)< 100) {
...
switch (conversation_choice) {
  ...
  case "4":
    ...
    break;
  default: //runs if none of the other conditions are met
